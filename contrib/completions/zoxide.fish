...
complete -c zoxide -n "__fish_zoxide_needs_command" -f -a "tidy" -d 'Repair and clean up database paths'
...
complete -c zoxide -n "__fish_zoxide_using_subcommand init" -l bind-fzf-insert -d 'Binds a control key (e.g. \'^g\') to interactively select and insert a directory' -r
...
complete -c zoxide -n "__fish_zoxide_using_subcommand tidy" -s d -l dedupe -d 'Merge different spellings of the same filesystem entry'
